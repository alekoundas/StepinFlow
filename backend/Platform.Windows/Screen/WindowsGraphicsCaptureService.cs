...
namespace Platform.Windows.Screen
{
    public sealed class WindowsGraphicsCaptureService : IWindowsGraphicsCaptureService, IDisposable
    {
        ...
        {
            IDirect3DDevice device = Direct3D11Interop.CreateDevice(out _devicePtr, out _contextPtr);
            _wrtDevice = device;
        }
        ...
        private byte[]? CaptureItem(GraphicsCaptureItem item, out int width, out int height)
        {
            ...
            using (capturedFrame)
            {
                return Direct3D11Interop.ReadStagingBytes(capturedFrame.Surface, _devicePtr, _contextPtr, width, height);
            }
        }
