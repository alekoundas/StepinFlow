...
namespace Business.Services.FlowScriptService
{
    ...
    ///
    /// The writer is a pure function over <see cref="FlowScriptSource"/>; this is the part that
    ...
    public sealed class FlowScriptExporter : IFlowScriptExporter
    {
        ...
        private readonly IFlowScriptWriter _writer;

        public FlowScriptExporter(IDbContextFactory<AppDbContext> dbContextFactory, IFlowScriptWriter writer)
        {
        ...
        public async Task<string> RenderAsync(int flowId, CancellationToken ct = default)
        {
            ...
            FlowScriptSource source = await BuildSourceAsync(dbContext, flowId, ct);
        ...
        public async Task<FlowExportResultDto> ExportAsync(int flowId, string? folderPath = null, CancellationToken ct = default)
        {
            ...
            // LF, because the round trip compares bytes and CI runs on Linux.
        ...
        private static async Task<FlowScriptSource> BuildSourceAsync(AppDbContext dbContext, int flowId, CancellationToken ct)
        {
            Flow flow = await dbContext.Flows
                .AsNoTracking()
                .FirstOrDefaultAsync(x => x.Id == flowId, ct)
            ...
            return new FlowScriptSource
            {
                ...
                TemplateFileNamesByStepId = TemplateFileNames(
                    templates.Select(x => (x.Id, x.FlowStepId, x.Name)).ToList(), stepNames),
            };
        }


        // ================================================================
        // Private methods - template file names
        // ================================================================
        ...
        /// repository is for.
        /// </summary>
        private static Dictionary<int, IReadOnlyList<string>> TemplateFileNames(
            IReadOnlyList<(int Id, int FlowStepId, string Name)> templates,
            IReadOnlyDictionary<int, string> stepNames)
        {
        ...
        private static async Task<int> WriteTemplatesAsync(AppDbContext dbContext, int flowId, FlowScriptSource source, string templateFolder, CancellationToken ct)
        {
        ...
        // A name a file system will accept, and never empty. Whitespace becomes a hyphen: these end
        // up as paths in a repository and inside the script, where a space costs quoting in every
        // shell and CI file that touches them.
