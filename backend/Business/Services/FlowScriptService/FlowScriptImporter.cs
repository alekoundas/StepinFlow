...
namespace Business.Services.FlowScriptService
{
    ...
    public sealed class FlowScriptImporter : IFlowScriptImporter
    {
        ...
        private readonly IFlowScriptReader _reader;

        public FlowScriptImporter(IDbContextFactory<AppDbContext> dbContextFactory, IFlowScriptReader reader)
        {
        ...
        public async Task<FlowImportResultDto> ImportAsync(string scriptPath, CancellationToken ct = default)
        {
            if (!File.Exists(scriptPath))
                return Failed(new FlowScriptError(1, 1, $"There is no file at {scriptPath}."));
        ...
        {
            FlowScriptDocument document = _reader.Read(script);
            if (!document.IsValid)
                return Failed(document.Errors);

            List<FlowScriptError> errors = new List<FlowScriptError>();
            FlowScriptSource source = FlowScriptResolver.Resolve(document, errors);
            if (errors.Count > 0)
        ...
        private static async Task<Dictionary<int, int>> WriteAreasAsync(
            AppDbContext dbContext, Flow flow, FlowScriptDocument document, CancellationToken ct)
        {
            Dictionary<int, int> ids = new Dictionary<int, int>();

            // Two passes so a child can be given a parent that already has a row.
            foreach (ParsedArea parsed in document.Areas.OrderBy(x => x.ParentName == null ? 0 : 1))
            {
        ...
        private static async Task<Dictionary<int, int>> WritePointsAsync(
            AppDbContext dbContext, Flow flow, FlowScriptDocument document, IReadOnlyDictionary<int, int> areaIds, CancellationToken ct)
        {
            Dictionary<int, int> ids = new Dictionary<int, int>();
        ...
        private static void ResolveReferences(FlowScriptDocument document, IReadOnlyDictionary<int, int> stepIds)
        {
            Dictionary<string, int> byName = new Dictionary<string, int>(StringComparer.Ordinal);

            foreach (ParsedStep parsed in document.Steps.Where(x => !string.IsNullOrEmpty(x.Step.Name)))
        ...
        private static FlowImportResultDto Failed(params FlowScriptError[] errors)
        {
            return Failed((IReadOnlyList<FlowScriptError>)errors);
        }

        private static FlowImportResultDto Failed(IReadOnlyList<FlowScriptError> errors)
        {
            return new FlowImportResultDto
            {
                IsSuccess = false,
                Errors = errors
                    .Select(x => new FlowScriptErrorDto { Line = x.Line, Column = x.Column, Message = x.Message })
                    .ToList(),
            };
        }
    }
